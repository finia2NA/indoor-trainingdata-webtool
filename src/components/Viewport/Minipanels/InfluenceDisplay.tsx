...
const InfluenceDisplay = ({ fullInfluenceUntil, zeroInfluenceAt }: InfluenceDisplayProps) => {
  ...
  const functionData = useMemo(() => {
    ...
    for (let i = 0; i <= numPoints; i++) {
      ...
      // Scale for display: x = distance mapped to -9 to 9, y = influence mapped to 0 to 1 (normalized)
      const x = ((distance - minDistance) / totalRange) * 18 - 9;
      const y = influence; // Keep influence as 0-1 range for proper scaling
  ...
  return (
    <Canvas className='bg-black' style={{ width: '100%', height: '50px' }}>
      ...
      <Line points={[new Vector3(-9, 0, 0), new Vector3(-9, 1, 0)]} color="blue" lineWidth={1} />

      {/* Influence function curve */}
      <Line points={functionData.points} color="white" lineWidth={2} />

      {/* X-axis distance labels */}
      <Html position={[-7.2, 1.5, 0]} center>
        <div style={{ color: 'skyblue', fontSize: '0.7rem', fontWeight: 'bold' }}>
          -0.5m
        </div>
      </Html>
      ...
      <Html position={[
        Math.max(-7, Math.min(7, ((fullInfluenceUntil - functionData.minDistance) / functionData.totalRange) * 18 - 9)),
        -0.15,
        0
      ]} center>
      ...
      <Html position={[
        Math.max(-7, Math.min(7, ((zeroInfluenceAt - functionData.minDistance) / functionData.totalRange) * 18 - 9)),
        0.5,
        0
      ]} center>
        <div style={{ color: 'violet', fontSize: '0.7rem', fontWeight: 'bold' }}>
      ...
      <Line
        points={[
          ...
          new Vector3(((fullInfluenceUntil - functionData.minDistance) / functionData.totalRange) * 18 - 9, 1, 0)
        ]}
        color="violet"
        lineWidth={1}
        opacity={0.5}
      ...
      <Line
        points={[
          ...
          new Vector3(((zeroInfluenceAt - functionData.minDistance) / functionData.totalRange) * 18 - 9, 1, 0)
        ]}
        color="violet"
        lineWidth={1}
        opacity={0.5}
